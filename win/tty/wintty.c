...
char * plbuf;
{
	...
	if (n > 1) {
		...
		if (ok_race(flags.initrole, i, flags.initgend,
		    ...
		    /* 
		     * Ask order patch, for compatibility with SLASH'EM.
		     * If the capitalized first letter is already used, 
		     * use morech
		     */
		    if (thisch == lastch) thisch = highc(thisch);
		    add_menu(win, NO_GLYPH, &any, thisch,
...
#if 1 /* This version deals with more than 2 races per letter */
