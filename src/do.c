...
giveback:
		...
		return;
	    case RIN_LEVITATION:
		pline_The("sink %s upward for a moment.", Hallucination ? "flies" : "quivers");
	    case RIN_POISON_RESISTANCE:
		You("smell %s %s.", Hallucination ? "fresh" : "rotten", makeplural(fruitname(FALSE)));
		break;
...
/* KMH, balance patch -- now an amulet
		...
		pline_The("water flow seems %s.", Hallucination ? "normal" : "fixed");
		break;
		...
		pline_The("water flow seems %s now.",
			Hallucination ? 
			((obj->spe<0) ? "fragile" : "tough") :
			((obj->spe<0) ? "lesser" : "greater"));
		break;
	    case RIN_GAIN_INTELLIGENCE:
	    case RIN_GAIN_WISDOM:
		pline("The water flow seems %s now.",
			Hallucination ? 
			((obj->spe<0) ? "foolish" : "wise") :
	...
	if(!Blind && !ideed && obj->otyp != RIN_HUNGER) {
		...
		case RIN_ADORNMENT:
		    if (!Hallucination) pline_The("faucets flash brightly for a moment.");
		    else You_feel("very attracted to the sink.");
		    break;
		...
		pline_The("water flow seems %s now.", Hallucination ?
				"sadder" : "cooler");
		    break;
	    ...
	    case RIN_TELEPORTATION:
		{
			...
			cy = rn2(ROWNO);
			trp = t_at(cx,cy);
		    } while (((levl[cx][cy].typ != ROOM) || (trp) || cansee(cx,cy)) && (cnt++ < 200));
		...
		pline_The("sink transforms into %s!", (levl[u.ux][u.uy].typ == THRONE)
		    ? "a throne" : 
		    (levl[u.ux][u.uy].typ == TOILET ? "a toilet" : an(surface(u.ux, u.uy))));
		newsym(u.ux,u.uy);
		trycall(obj);
		useup(obj);
		return;
		...
		case RIN_POLYMORPH_CONTROL:
			...
			case 0:
			    ...
			    break;
			case 1:
			    pline_The("sink momentarily looks like a throne.");
			    break;
			case 2:
			    pline_The("sink momentarily looks like an altar.");
			    break;
			case 3:
			    pline_The("sink momentarily looks like a toilet.");
		    }
		    break;
	    }
	}
...
extern boolean castle_hor_flip, castle_ver_flip;
	...
	if (portal && !In_endgame(&u.uz)) {
		...
		if (at_ladder) {
			...
			if (Is_stronghold(&u.uz)) {
				...
				if(!castle_ver_flip) y = rn1(ROWNO - 4, 3);
				else y = rn1(ROWNO - 4, 2);
			    } while(occupied(x, y) ||
