	...
	register struct obj	*otmp;
{
	...
	case POT_PARALYSIS:
		if (Free_action)
		    ...
		    nomul(-(rn1(10, 25 - 12*bcsign(otmp))), "paralyzed by a potion");
...
register struct obj *obj;
{
	...
	case POT_PARALYSIS:
		...
		if (!Free_action) {
		    ...
		    nomul(-rnd(5), "paralyzed by a potion");
		    nomovemsg = You_can_move_again;
		    exercise(A_DEX, FALSE);
	...
	case POT_SLEEPING:
		...
		if (!Free_action && !Sleep_resistance) {
		    ...
		    nomul(-rnd(PSleep_resistance ? 2 : 5), "asleep");
		    nomovemsg = You_can_move_again;
		    exercise(A_DEX, FALSE);
		} else You("yawn.");
		break;
 ...
 */
{
	...
	switch (obj->otyp)
	{
		...
		case DARK_ELVEN_DAGGER:
			obj->otyp = rn2(2) ? GREAT_DAGGER : WOODEN_STAKE;
			break;
		case KNIFE:
			if (!rn2(2)) obj->otyp = SCALPEL;
			else obj->otyp = STILETTO;
			break;
		case SCALPEL:
		case STILETTO:
			obj->otyp = KNIFE;
		...
		case HIGH_BOOTS:
			...
			do {
				 ...
				 * 1. Speed boots are the first boots in onames.h
				 * 2. Ring of adornment is the first ring.
				 * 3. Rings are directly after boots
				 */
				obj->otyp = rn1(RIN_ADORNMENT - SPEED_BOOTS, SPEED_BOOTS);
			} while(obj->otyp == FUMBLE_BOOTS ||
				objects[obj->otyp].oc_material != LEATHER);
			break;
		case IRON_SHOES:
			if (objects[FUMBLE_BOOTS].oc_material == IRON)
				return 0;
			else {
				/*
				 * Assume there's only one type of iron magic boots...
				 */
				for (obj->otyp = SPEED_BOOTS;
				     obj->otyp < RIN_ADORNMENT;
				     obj->otyp++)
					if (objects[obj->otyp].oc_material == IRON)
						break;
				if (obj->otyp >= RIN_ADORNMENT) {
					impossible("no iron magic boots?");
					return 0;
				}
			}
			break;
		...
		case FLINT:
			switch (rn2(4)) {
				case 0: obj->otyp = LUCKSTONE; break;
				case 1: obj->otyp = HEALTHSTONE; break;
				case 2: obj->otyp = TOUCHSTONE; break;
				case 3: obj->otyp = WHETSTONE; break;
			}
			break;
		case TOUCHSTONE:
		case WHETSTONE:
			obj->otyp = rn2(2) ? LUCKSTONE : HEALTHSTONE;
			break;
		case LUCKSTONE:
		case HEALTHSTONE:
			obj->otyp = rn2(2) ? TOUCHSTONE : WHETSTONE;
			break;
		default:
