...
dig()
{
	...
	if (digging.down) {
		...
		if (dighole(TRUE, FALSE, (coord *)0)) {	/* make pit at <u.ux,u.uy> */
...
coord *cc;
{
	...
	if ((ttmp && (ttmp->ttyp == MAGIC_PORTAL ||
	   ...
	   /* ALI - artifact doors */
	   IS_DOOR(lev->typ) && artifact_door(dig_x, dig_y) ||
	...
	/* the following two are here for the wand of digging */
		...
		typ = fillholetyp(u.ux,u.uy,FALSE);
...
coord *cc;
{
	...
	case 2:
	    if (!Blind) pline(Hallucination ? "Dude!  The living dead!" :
 	    ...
 	    mtmp = makemon(mkclass(S_ZOMBIE,0), dig_x, dig_y, NO_MM_FLAGS);
	    break;
	case 3:
	    if (!Blind) pline(Hallucination ? "I want my mummy!" :
 			"You've disturbed a tomb!");
 	    mtmp = makemon(mkclass(S_MUMMY,0), dig_x, dig_y, NO_MM_FLAGS);
	    break;
	default:
	    /* No corpse */
...
/*        struct obj *otmp;*/
	...
	if (u.uswallow) {
		...
		mtmp->mhp = (mtmp->mhp + 1) / 2;	/* that hurts */
