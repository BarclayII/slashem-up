 ...
 */
    /* Long worms are more dangerous now if TOUGH_LONGWORM is enabled :P */
    for (seg = wtails[wnum]; seg; seg = seg->nseg)
#ifndef TOUGH_LONGWORM
    ...
    struct obj *weap;
{
    ...
    /* OK, *occasionally* the tail end dies. */
#ifdef TOUGH_LONGWORM
