...
register int x, y, typ;
{
	...
	switch(typ) {
		...
		mtmp = makemon(&mons[statue->corpsenm], 0, 0, MM_IGNOREWATER);
...
long hmask, emask;     /* might cancel timeout */
{
	...
	if (!trap) {
	    trap = t_at(u.ux,u.uy);
	    if(Is_airlevel(&u.uz))
		You((Flying) ? "feel less buoyant." :
				"begin to tumble in place.");
...
#endif
		{
			...
			if (!sokoban_trap)
			    You((!Flying)?"float gently to the %s.":
					    "feel less buoyant.",
				surface(u.ux, u.uy));
			else {
...
climb_pit()
{
    ...
    } else if ((Flying || is_clinger(youmonst.data)) && !In_sokoban(&u.uz)) {
