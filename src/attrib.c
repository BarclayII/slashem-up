			...
			if (uarmg && uarmg->otyp == GAUNTLETS_OF_POWER)
				if ((uarmg->spe > 7)
						|| ((118 + bonus + uarmg->spe) > 125)
		   				|| ((tmp + uarmg->spe) > 125))
	   				resg = 125;
				else if (base > 118) resg = base + uarmg->spe + bonus;
				else resg = 118 + uarmg->spe + bonus;
			if (uwep && uwep->oartifact == ART_SCEPTRE_OF_MIGHT)
				if ((uwep->spe > 7)
						|| ((118 + bonus + uwep->spe) > 125)
		   				|| ((tmp + uwep->spe) > 125))
	   				resw = 125;
				else if (base > 118) resw = base + uwep->spe + bonus;
				else resw = 118 + uwep->spe + bonus;
			if (resg > resw)
				return resg;
			else
				return resw;
