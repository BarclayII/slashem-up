...
STATIC_DCL void NDECL(check_lunar);
STATIC_DCL void NDECL(check_solar);
STATIC_DCL void NDECL(new_turn_setup);
STATIC_DCL void NDECL(post_turn);
STATIC_DCL void NDECL(post_input);

static int moveamt = 0;		/* move amount */
static int wtcap = 0;		/* weight capability */
static int change = 0;		/* polymorphed */
static boolean didmove = FALSE;	/* (tried to) move */
static boolean monscanmove = FALSE;
...
/* check and set appropriate moon phase flags */
STATIC_DCL void
check_lunar()
{
...
/* [BarclayII] once-per-hero-took-time things go here */
void
post_turn()
{
	...
	if(!u.uinvulnerable) {
		if(Teleportation && !rn2(85)) {
			...
			if (u.ux != old_ux || u.uy != old_uy) {
				if (!next_to_u()) {
					check_leash(&youmonst, old_ux, old_uy,
						    TRUE);
				}
#ifdef REDO
