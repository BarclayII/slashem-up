...
/*	if (level.flags.arboreal)
	...
	if(In_sokoban(&u.uz)) {
		...
		/* Clipped from UnNetHack, but only generates AoR, BoH and CoMR */
		...
		if (dunlev(&u.uz)==1) {
			...
			for (ep = head_engr; ep; ep = ep->nxt_engr) {
				...
				if (ep && ep->engr_txt[0] && ep->engr_type == BURN &&
					...
					/* And Sokoban prizes are never blessed or
					 * have positive enchantment */
					cancel_item(otmp);
		...
		if (dunlev(&u.uz)==1) {
			...
			if (ep && ep->engr_txt[0] && ep->engr_type == BURN &&
				...
				int otyp;
				switch(rn2(3)) {
					case 0: otyp = BAG_OF_HOLDING; break;
					case 1: otyp = AMULET_OF_REFLECTION; break;
					case 2: otyp = CLOAK_OF_MAGIC_RESISTANCE; break;
				}
				otmp = mksobj_at(otyp, ep->engr_x, ep->engr_y, TRUE, FALSE);
				cancel_item(otmp);
			}
		}
#endif
