	...
	if(u.uhunger < 1500 && !u.uswallow) {
		...
		switch(mattk->adtyp) {
			...
			if (mdef->mhp > 0) { /* monster lifesaved */
				...
				nomul(-tmp, "digesting %s", a_monnam_or_sth(mdef));
				nomovemsg = msgbuf;
			    } else pline("%s", msgbuf);
	 ...
	 */
		...
		case AT_BITE:
			...
			if ((uwep || u.twoweap && uswapwep) &&
			    ...
			    break;
			/* Vampires attacking petrifying monsters with
			 * gloved hands still bite them, which is weird.
			 */
		...
		if(ptr == &mons[PM_FLOATING_EYE]) {
			...
			if (ureflects("%s gaze is reflected by your %s.",
			    ...
			    You("are frozen by %s gaze!",
				  s_suffix(mon_nam(mon)));
			    nomul((ACURR(A_WIS) > 12 || rn2(4)) ? -tmp : -127, "frozen by %s", a_monnam_or_sth(mon));
			}
		    } else {
		    ...
		    nomul(-tmp, "frozen by %s", a_monnam_or_sth(mon));
