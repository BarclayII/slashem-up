	...
	case SPE_FORCE_BOLT:
		...
		if (resists_magm(mtmp)) {	/* match effect on player */
			...
			break;	/* skip makeknown */
                } else if (mtmp->data == &mons[PM_STONE_GOLEM] ||
                           mtmp->data == &mons[PM_GLASS_GOLEM] ||
                           mtmp->data == &mons[PM_CLAY_GOLEM]) {
                        if (canseemon(mtmp))
                           pline("%s shatters!", Monnam(mtmp));
			else if (flags.soundok)
			   You_hear("something shatter.");
                        if (mtmp->data == &mons[PM_STONE_GOLEM]) {
                           if (m_using)
                                monkilled(mtmp, (char *)0, AD_RBRE);
                           else
                                xkilled(mtmp,2);
                           obj = mksobj_at(ROCK, mtmp->mx, mtmp->my, FALSE, FALSE);
                           obj->quan = (long)(rn2(20) + 50);
                           obj->owt = weight(obj);
                        } else {
                           if (m_using)
                                monkilled(mtmp, (char *)0, AD_SPEL);
                           else
                                xkilled(mtmp,0);
                        }
...
boolean ordinary;
{
	...
	switch(obj->otyp) {
		...
		case SPE_FORCE_BOLT:
			...
			shieldeff(u.ux, u.uy);
			pline("Boing!");
                    } else if (u.umonnum == PM_STONE_GOLEM ||
                               u.umonnum == PM_GLASS_GOLEM ||
                               u.umonnum == PM_CLAY_GOLEM) {
                        You("shatter!");
                        u.mh = -1;
                        rehumanize();
		    } else {
...
boolean			youattack, allow_cancel_kill, self_cancel;
{
	...
	if (youdefend) {
		...
		pline("%s is %s", Monnam(mdef),
			!Hallucination ? "covered in sparkling lights!"
			      : "enveloped by psychedelic fireworks!");
 ...
 */
struct monst *
bhit(ddx,ddy,range,weapon,fhitm,fhito,obj_p,obj_destroyed)
...
char oclass;
{
	...
	return (rn2(100 + alev - dlev) < mtmp->data->mr);
}
...
int damage, tell;
{
	...
	resisted = mon_resist(mtmp, oclass);
	if (resisted) {
	    if (tell) {
		shieldeff(mtmp->mx, mtmp->my);
