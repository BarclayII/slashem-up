...
int dieroll; /* needed for Magicbane and vorpal blades */
{
	...
	*dmgptr += spec_dbon(otmp, mdef, *dmgptr);
	...
	if (attacks(AD_STUN, otmp) &&
			dieroll <= MB_MAX_DIEROLL) {
 ...
 */
	...
	case SCARING:
	    ...
	    if (!Blind)
		pline("You unleash a black aura from %s!", the(xname(obj)));
	    else
		pline("%s vibrates violently!", The(xname(obj)));
	    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
		if (DEADMONSTER(mtmp)) continue;
		/* Monsters who can't see you are not affected */
		if (!m_cansee(mtmp, u.ux, u.uy)) continue;
		/* Peaceful monsters are not affected. */
		if (mtmp->mpeaceful) continue;
		/* Mindless creatures won't have nightmares */
		if (mindless(mtmp->data)) continue;
		/* Sleeping monsters are waked up by nightmare */
		if (mtmp->msleeping) {
			mtmp->msleeping = 0;
			if (canseemon(mtmp))
				pline("%s suddenly wakes up from the nightmare.", Monnam(mtmp));
		}else if ((distu(mtmp->mx, mtmp->my) <= 9*9) &&
		    (!mon_resist(mtmp, TOOL_CLASS))){
			if (canseemon(mtmp)) pline("%s is frightened to death and unable to move!", Monnam(mtmp));
			mtmp->mcanmove = 0;
			mtmp->mfrozen = rnd(50);
		}else
			monflee(mtmp, rnd(50), FALSE, TRUE);
	    }
	    /* Tsk,tsk.. (?) */
	    adjalign(-9);
	    u.uluck -= 3;
	...
	case SUMMON_FIRE_ELEMENTAL:
	    ...
	    if (!Role_if(PM_FLAME_MAGE) && !Role_if(PM_ICE_MAGE)) {
		    pline("You don't know how to use this artifact.");
		    break;
	    }
	    /* The lawful god of flame mages is called *Earth*, right? */
	    if (Confusion)
	...
	case SUMMON_WATER_ELEMENTAL:
	    ...
	    /* And for ice mages he is called *Air*? */
	    if (Confusion)
