...
/*
 * Castle level upstairs flip patch
 */
boolean castle_hor_flip = FALSE;
boolean castle_ver_flip = FALSE;
...
int flp;
{
	...
	for (y = 1; y <= y2 - 1; ++y) {
		...
		tmpdbdir = levl[x][y].drawbridgemask & DB_DIR;
		levl[x][y].drawbridgemask ^= tmpdbdir;
		switch(tmpdbdir){
		    case DB_NORTH:
			levl[x][y].drawbridgemask ^= (flp & 1) ? DB_SOUTH : DB_NORTH;
			break;
		    case DB_SOUTH:
			levl[x][y].drawbridgemask ^= (flp & 1) ? DB_NORTH : DB_SOUTH;
			break;
		    case DB_EAST:
			levl[x][y].drawbridgemask ^= (flp & 2) ? DB_WEST : DB_EAST;
			break;
		    default:
			impossible("DRAWBRIDGE FLIPPING FAILED???");
		    case DB_WEST:
			levl[x][y].drawbridgemask ^= (flp & 2) ? DB_EAST : DB_WEST;
			break;
		}
	    }
	}
...
dlb *fd;
{
    ...
    if (rn2(2)) {
		...
		castle_ver_flip = TRUE;
    }
    if (rn2(2)){
	    c |= 2;
	    if(Is_stronghold(&u.uz))
		    castle_hor_flip = TRUE;
    }
    flip_level(c);
#endif
    return TRUE;
